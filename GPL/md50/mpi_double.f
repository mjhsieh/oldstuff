...
*     Uses double precision in the data transfer
*
...
C    and then broadcast them
          call MPI_ALLGATHERV(BUFF,NPC3,MPI_DOUBLE_PRECISION,BUF2,NAP3,
     +NABS3,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr)
C   This normally not necessary
          call MPI_BARRIER(MPI_COMM_WORLD,IERR)
...
*    2.2.1 Write in buffer
	  ...
	  END DO 
*    2.2.2 Communicate
        call MPI_ALLGATHER(BUFF,NAPC3,MPI_DOUBLE_PRECISION,BUF2,NAPC3,
...
C   This is only to ensure that they are the same on all the nodes
	...
	call MPI_BCAST(BUFF,8,MPI_DOUBLE_PRECISION,MAST,
     +MPI_COMM_WORLD,ie)	
	...
	if(LAVER)then 
	  ...
	  if(ICNT.gt.NBUFF)then
	    ...
	    call FINAL
          ...
          call MPI_REDUCE(BUFF,BUF2,ICNT,MPI_DOUBLE_PRECISION,MPI_SUM,
...
*   ------------------     
 	do J=0,NUMTASK-1
      ...
      call MPI_REDUCE_SCATTER(BUFF,BUF2,NAP3,MPI_DOUBLE_PRECISION,
