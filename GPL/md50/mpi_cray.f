...
*    2.1.3 Reconstruct coordinates from the buffer 
	  do K=0,NUMTASK-1
C    Do not overwrite own coordinates
	    if(K.ne.TASKID)then 
	      NPC=NAP(K)
              do J=NAB(K),NAE(K)  
	        I=J+2*NABS(K)
...
*   ---------------------------------------------------
	if(IND.eq.1)then 
	  NAPP=NAP(TASKID)             
	  call MPI_GATHERV(VX(ISH),NAPP,MPI_REAL,VX,
     +NAP,NABS,MPI_REAL,MAST,MPI_COMM_WORLD,ie)
	  call MPI_GATHERV(VY(ISH),NAPP,MPI_REAL,VY,
     +NAP,NABS,MPI_REAL,MAST,MPI_COMM_WORLD,ie)
	  call MPI_GATHERV(VZ(ISH),NAPP,MPI_REAL,VZ,
     +NAP,NABS,MPI_REAL,MAST,MPI_COMM_WORLD,ie)
*
*   3.2 Case of equal number of atoms on processors
*   -----------------------------------------------
	else   ! IND=2
	  call MPI_GATHER(VX(ISH),NAPC,MPI_REAL,VX,
     +NAPC,MPI_REAL,MAST,MPI_COMM_WORLD,ie)
	  call MPI_GATHER(VY(ISH),NAPC,MPI_REAL,VY,
     +NAPC,MPI_REAL,MAST,MPI_COMM_WORLD,ie)
	  call MPI_GATHER(VZ(ISH),NAPC,MPI_REAL,VZ,
     +NAPC,MPI_REAL,MAST,MPI_COMM_WORLD,ie) 
	end if
* 
...
*     
	...
	real*8 A(N),B(N)
	include "mpif.h"
	  call MPI_ALLREDUCE(A,B,N,MPI_REAL,MPI_SUM,
     $     MPI_COMM_WORLD,ierr)     
	return
	end
*
...
*                                                                  
	...
	real*8 A(N),B(N)
	include "mpif.h"
      call MPI_REDUCE(A,B,N,MPI_REAL,MPI_SUM,
     $     MAST,MPI_COMM_WORLD,ierr)
	return
	end   
*
...
*                                                                  
      ...
      real*8 A(N)
      include "mpif.h"
      call MPI_BCAST(A,N,MPI_REAL,MAST,MPI_COMM_WORLD,ie)	
      return
      end   
*
*========================== BARRIER ================================
