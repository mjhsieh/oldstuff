...
*     Additional control of data transfer 
*     (for buggy clusters)
*
...
C   This is number atoms on the processor    
	  ...
	  do K=1,NPC    
	    ...
	    BUFF(K+NPC)=SY(I)
	    BUFF(K+2*NPC)=SZ(I)
            if(abs(BUFF(K)).gt.1.5*BOXL.or.abs(BUFF(K+NAPC)).gt.
     + 1.5*BOYL.or.abs(BUFF(K+2*NAPC)).gt.1.5*BOZL)then
                 write(*,*)' !!!OBS: node ',TASKID,
     +' has strange coordinates ',I,' step ',MSTEP,NAPC
           write(*,*)'for atom ',I+ISH,SX(I+ISH),SY(I+ISH),SZ(I+ISH)
            end if
	  end do
*    2.1.2 Communicate
	  ...
	  do K=0,NUMTASK-1
	      NPC=NAP(K)
	        ...
	        I=J+2*NABS(K)
                ...
                DZ=SZ(J)-BUF2(I3)
                call PBC(DX,DY,DZ)
              if(abs(DX).gt.1..or.abs(DY).gt.1..or.abs(DZ).gt.1.)then
                 write(*,*)' !!!OBS: node ',TASKID,
     +' get strange coordinates from node ',K,' step ',MSTEP
                 write(*,*)'Was: atom ',J,SX(J),SY(J),SZ(J)
                 write(*,*)'Get: atom ',J,BUF2(I),BUF2(I2),BUF2(I3)
                 write(*,*)'Continue with old coordinates'
*                 close(6)
*                 open(6,access='append')
              else
	        SX(J)=BUF2(I)
	        SY(J)=BUF2(I2)
	        SZ(J)=BUF2(I3)
...
*    2.2.1 Write in buffer
	  do I=1,NAPC    
	    ...
	    BUFF(I+2*NAPC)=SZ(I+ISH)
            if(abs(BUFF(I)).gt.1.5*BOXL.or.abs(BUFF(I+NAPC)).gt.
     + 1.5*BOYL.or.abs(BUFF(I+2*NAPC)).gt.1.5*BOZL)then
                 write(*,*)' !!!OBS: node ',TASKID,
     +' has strange coordinates ',I,' step ',MSTEP,NAPC
           write(*,*)'for atom ',I+ISH,SX(I+ISH),SY(I+ISH),SZ(I+ISH)
*                 close(6)
...
*    2.2.2 Communicate
     ...
     +  MPI_REAL,MPI_COMM_WORLD,ierr)
        if(ierr.ne.0)then
          write(*,*)' Obs!!! Error in MPI_ALLGATHER,  step',
     +    MSTEP,' node ',TASKID,' ierr=',ierr
        end if
*    2.2.3 Restore coordinates from the buffer
	  do K=0,NUMTASK-1
	      ...
	      I=J+2*NABS(K)
              I2=I+NAPC
              I3=I2+NAPC
*              if(abs(BUF2(I)).gt.1.5*BOXL.or.abs(BUF2(I2)).gt.
*     + 1.5*BOYL.or.abs(BUF2(I3)).gt.1.5*BOZL)then
              DX=SX(J)-BUF2(I)
              DY=SY(J)-BUF2(I2)
              DZ=SZ(J)-BUF2(I3)
              call PBC(DX,DY,DZ)
...
C   This is only to ensure that they are the same on all the nodes
	...
	call MPI_BCAST(BUFF,9,MPI_REAL,MAST,
     +MPI_COMM_WORLD,ie)	
...
*============================ BCASTI ===============================
*                                                                  
*      subroutine BCASTI(I,N,MAST)
*      integer I(N)
*      include "mpif.h"
*      call MPI_BCAST(I,N,MPI_INTEGER,MAST,MPI_COMM_WORLD,ie)	
*      return
*      end   
*
*========================== BARRIER ================================
...
*        call MPI_BARRIER(MPI_COMM_WORLD,IERR)
	call MPI_FINALIZE(ie)
        write(*,*)' Node ',TASKID,': Program finalized'
        close(6)
	stop ''
	end
